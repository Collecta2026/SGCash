...
T = {
    ...
    "available_cash": ("Available cash", "النقدية المتاحة"),
    "committed_out": ("Committed to go out ({n} weeks)", "مدفوعات ملتزم بها ({n} أسابيع)"),
    "headroom": ("Headroom", "الفائض"),
    "health_green": ("Comfortable — available cash covers what is committed",
                     "مريح — النقدية المتاحة تغطي الالتزامات"),
    "health_amber": ("Tight — headroom of {pct} over committed payments",
                     "ضيق — الفائض {pct} فقط فوق المدفوعات الملتزم بها"),
    "health_amber_buffer": ("Tight — the balance dips below the minimum buffer",
                            "ضيق — الرصيد ينخفض دون الحد الأدنى"),
    "health_red": ("Shortage forecast — cash runs out in {week}",
                   "عجز متوقع — تنفد النقدية في {week}"),
    "health_red_cover": ("Shortage — available cash does not cover committed payments",
                         "عجز — النقدية المتاحة لا تغطي المدفوعات الملتزم بها"),
    "awaiting_review": ("Awaiting review", "بانتظار المراجعة"),
    "entries_submitted": ("entries submitted for approval", "سجل مُرسل للاعتماد"),
